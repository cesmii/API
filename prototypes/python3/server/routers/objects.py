...
def update_object(
    update: UpdateRequest = Body(...),
    data_source: I3XDataSource = Depends(get_data_source),
):
    if len(update.elementIds) != len(update.values):
        raise HTTPException(
            status_code=400,
            detail="elementIds and values arrays must be of the same length",
        )
...
@objects.get(
    ...
    response_model=List[HistoricalValue],
    tags=["Objects"],
)
def get_historical_values(
    ...
    instance = data_source.get_instance_values_by_id(elementId, startTime, endTime)
    if instance:
